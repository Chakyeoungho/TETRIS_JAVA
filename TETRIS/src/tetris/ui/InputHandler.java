...
public class InputHandler extends KeyAdapter {
	...
	public void keyPressed(KeyEvent e) {
		...
		case KeyEvent.VK_DOWN:
			if (isDownPressed)
				break; // 꾸 눌러도 처음 한번만 인식
			isDownPressed = true; // 누름 상태
			gameEngine.setIntervalNanos(gameEngine.getDropTime(gameEngine.getScoreManager().getLevel()) / 20); // 소프트드랍
																												// 시간 설정
	...
	public void keyReleased(KeyEvent e) {
		if (e.getKeyCode() == KeyEvent.VK_DOWN) {
			...
			gameEngine.setIntervalNanos(gameEngine.getDropTime(gameEngine.getScoreManager().getLevel())); // 메인타이머
																											// 원래시간으로
																											// 초기화
	...
	public void resetLockDelayCounter() {
		...
		if (gameEngine.isLockDelayRunning() && gameEngine.getLockDelayCounter() < 15) {
			if (!canFall) gameEngine.resetLockDelay();
			else gameEngine.restartMainTimer();
